...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return !(rect1.left > width2 || rect2.left > width1
    || rect1.top > height2 || rect2.top > height1);
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  return ((isStartIncluded) ? '[' : '(').concat((a < b) ? `${a}, ${b}` : `${b}, ${a}`)
    .concat((isEndIncluded) ? ']' : ')');
}
...
function getDigitalRoot(num) {
  if (num < 9) return num;
  num = parseInt(num.toString().split('').reduce((a, b) => Number(a) + Number(b)), 10);
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[0][0] === position[1][1]
    && position[1][1] === position[2][2] && position[2][2] !== undefined) return position[0][0];
  if (position[0][2] === position[1][1]
    && position[1][1] === position[2][0] && position[2][2] !== undefined) return position[1][1];
