...
function extractNameFromTemplate(value) {
  return value.split(',')[1].slice(0, -1).trimStart();
}
...
function getRectangleString(width, height) {
  const newLineLength = '\\n'.length;
  const topSide = `┌${'─'.repeat(width - newLineLength)}┐\n`;
  const middle = `│${' '.repeat(width - newLineLength)}│\n`.repeat(height - newLineLength);
  const bottomSide = `└${'─'.repeat(width - newLineLength)}┘\n`;
...
function encodeToRot13(str) {
  return str.split('').map((currentValue, i) => {
    ...
    const temp = charI % 32 <= 13
      ? String.fromCharCode(charI + 13)
      : String.fromCharCode(charI - 13);
